...
class Agent:
    ...
    def _load_model(self):
        model_path = './model/model_DQN/model_DQN.pth'
...
def train():
    # 检查是否需要加载模型进行断点续训
    load_model = os.path.exists('./model/model_DQN/model_DQN.pth')
    agent = Agent(load_model=load_model)
    game = SnakeGameAI(is_training=True)
    record = 0
    
    # 建立日志文件（处理断点续训时的日志覆盖问题）
    if not os.path.exists('./log/log_DQN'):
        os.makedirs('./log/log_DQN')
    
    # 检查是否是断点续训
    if agent.n_games > 0:
        # 读取现有日志文件，过滤掉轮数大于等于当前轮数的记录
        filtered_lines = []
        try:
            with open(f"./log/log_DQN/train_log_DQN.txt", "r") as f:
                for line in f:
                    # 提取轮数信息
                    if "Game: " in line:
        ...
        # 重写日志文件，只保留轮数小于当前轮数的记录
        log_file = open(f"./log/log_DQN/train_log_DQN.txt", "w")
        ...
        if done:
            ...
            if score > record:
                ...
                if score > agent.best_score:
                    ...
                    agent.model.save('best_model_DQN.pth', n_games=agent.n_games, best_score=agent.best_score)
    ...
    print("最好的模型已保存为 best_model_DQN.pth")
